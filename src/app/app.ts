...
export class App {
  ...
  reload() {
    ...
    this.api.list().pipe(finalize(() => this.loading.set(false))).subscribe({
      ...
      error: () => this.loadError.set('Could not load workflows. Check that the API and PostgreSQL are running, then retry.')
    });
  }
  ...
  create() {
    ...
    if (!name || name.length > 200 || this.description.length > 2000) {
      this.formError.set('Enter a name of 1–200 characters and a description of at most 2,000 characters.');
      return;
    }
    this.saving.set(true);
    this.api.create({ name, description: this.description.trim() || null })
      .pipe(finalize(() => this.saving.set(false))).subscribe({
        ...
        error: (error: HttpErrorResponse) => this.formError.set(error.status === 400
